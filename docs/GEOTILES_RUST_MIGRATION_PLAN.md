...
- [x] **§7.0 quality gates** pass for **`-p libgeotiles`** (license deny, typos, clippy both feature matrices, tests both matrices with `RUSTFLAGS='-D warnings'`).
- [x] `cargo build --release -p libgeotiles` with default features.
- [x] Library can produce a **valid** `{z}/{x}/{y}` tree (via API or **examples**/integration tests) from a **representative** GeoTIFF for at least **EPSG:4326** and **EPSG:3857** sources (after warp). (Verified end-to-end via `geotiles` binary which calls `libgeotiles` API directly; chunk-boundary panic fixed 2026-04-25.)
- [x] **Tile crop** is performed entirely inside `libgeotiles` (no external tool invocation for the crop step).
- [x] **Chunked I/O:** `chunk_size` configurable via `Settings` / CLI / config file (default 512 rows); pipeline reads one chunk at a time and releases the buffer before advancing; chunk read window expanded to cover the full source extent of all tiles in the chunk (fixes low-zoom boundary panic); safe default provided. (`GeoTiff` builder was removed in the architecture refactor — chunking is now a pipeline-level concern, not a builder setter.)
- [x] **No** dependency on abandoned crates (per §3 policy). (`cargo deny check licenses` passes; all dependencies have recent releases.)
- [x] **No** requirement for Avalonia, Docker, or GTiff2Tiles.Console parity.
- [x] **GPU path (Phase 7):** optional `gpu` features compile; CPU remains **default** features; optional manual/visual check vs CPU path for a few tiles. (Confirmed: `cargo build --features gpu --release` succeeds; `#[ignore]` GPU integration tests present in `src/tile/gpu/tests.rs`.)
- [x] **Output formats (Phase 6):** at least **PNG** on default features; **JPEG**, **WebP**, **AVIF**, **JXL** via **optional** features with **documented** encoder choices and packager notes. (PNG/JPEG/WebP via `image`; AVIF via `ravif` pure-Rust; JXL via `jxl-encoder` pure-Rust — no system `libjxl` required.)
- [x] **Testing (§6):** fixture manifest (`tests/fixtures_manifest.toml`) + cache helper (`tests/common/mod.rs::ensure_fixture`) present; default `cargo test` offline (all heavy tests are `#[ignore]`d); full suite command documented in manifest header (`GEOTILES_TEST_FETCH=1 cargo test -p libgeotiles -- --include-ignored`); synthetic micro-GeoTIFF tests in `tests/gdal_io.rs` and `tests/warp.rs`.
- [x] **Benchmarks (§6.6):** `criterion` benchmark suite present (`benches/encode.rs`, `benches/pipeline.rs`); baseline record template created in `docs/benchmarks/README.md` — **CPU and GPU baseline numbers must be filled in after first run on a stable machine** (see `docs/benchmarks/README.md`).
- [x] **Logging:** `tracing` spans / events present throughout the pipeline (open → warp → tile loop → encode → write); no phase may be merged without its log points.
- [x] **Repo automation (§2.1):** `.github/workflows`, Dependabot, `deny.toml`, `.typos.toml` adapted from tofi placeholder files (Phase 0) and aligned with **imgvwr** style (minimal renames + GDAL; jobs now use `--workspace` since `geotiles` binary exists; `libjxl` system package removed from all CI jobs — `jxl-encoder` is pure Rust).
...
- [x] End-to-end: **`geotiles`** produces tiles from argv + config on a real GeoTIFF. (Verified manually: `./geotiles -i Input4326.tif -o 4326_tiles --zoom 0..12` completes successfully; chunk-boundary panic at zoom 0 fixed 2026-04-25.)
...
| 2026-04-25 | `geotiles` source restructured: `cli.rs`/`config.rs`/`run.rs`/`tmr.rs` converted to `module/mod.rs` + `tests.rs` directory layout (plan §6.0 rule); new `settings/mod.rs` holds `Settings` struct (formerly `Params`) + `Crs` enum + all resolve/parse/build helpers; `main` calls `Settings::resolve(&cli, &config)` then `drop(cli); drop(config)` — only `settings` flows downstream; `logger/mod.rs` encapsulates `tracing_subscriber` init; `toml` bumped `"0.8"` → `"1"` (resolves to 1.1.2); `libjxl` system package removed from both `test.yml` and `fmt-clippy.yml` (jxl-encoder is pure Rust); `pipeline/grids.rs` imports gated with `#[cfg(any(feature = "geographic", feature = "mercator"))]` (fixes `--no-default-features` clippy/test failure) |
| 2026-04-25 | Chunk-boundary panic fixed: `run_zooms` now expands the GDAL read window to cover the deepest source row required by any tile in the chunk (`required_row_end = jobs.iter().map(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | j   | j.window.row + j.window.height).max().min(ds_h)`); without this, low-zoom tiles whose source window spans the full raster height (e.g. zoom 0 with ds_h=3511, chunk_size=512) indexed one past the end of the chunk buffer and panicked; all §7.0 gates pass |
| 2026-04-25 | §9 audit: all §9.1 and §9.2 items ticked; testing infrastructure added — `tests/fixtures_manifest.toml` (stable URLs + optional SHA-256, `ensure_fixture` resolution order documented), `tests/common/mod.rs` (`ensure_fixture` cache helper using `curl` + `sha256sum` subprocesses, `GEOTILES_TEST_DATA_DIR` / `GEOTILES_TEST_FETCH` env gates, `#[ignore]` pattern documented); `docs/benchmarks/README.md` created as baseline record template (CPU/GPU/encode tables; run instructions); §9.1 chunked-I/O item reworded to reflect architecture refactor (no `GeoTiff` builder — chunking is a pipeline-level concern via `Settings`)                                                                                                                       |
